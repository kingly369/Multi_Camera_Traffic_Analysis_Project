...
# Loads in Tensorrt model into pycuda
...
# Checks if the points intersect
...
# Changes the image format into letterbox mode
...
# Returns the Bounding Boxes, confidence score and class id from the frame
def get_boxes(frame):
    ...
    while counter < len(dettrt):
        if dettrt[counter][5] == 2:
            dettrt[counter][5] = 0
            counter = counter + 1
        elif dettrt[counter][5] in [7, 6, 5]: # If it's a large vehicle such as truck, boat, or train, we consider it to be a Truck class
            dettrt[counter][5] = 1
            counter = counter + 1
        else:
...
for video_id in range(int(camera_id), int(camera_id)+1):
    inside_polygon = []
    # Gets the ROI where both receiver and sender cameras can view the same intersection area
    if start_video == "15a":
        # cam 33 a
        ...
        print("Didn't annotate this camera ID yet")
        raise
    ...
    print("Track path", os.path.join(Track3_path, video_name))
    vs = cv2.VideoCapture(os.path.join(Track3_path, video_name))
    (W, H) = (None, None)
    writer = None
    last_frames = {}
    print("Waiting for Kent Jetson")
    channel = startupjack()
    print("Starting jack jetson")
    ...
    data = {}
    frame_count = 0
    # Creates the output video. Remove the variables fourcc, output_video, etc to increase the fps
    ...
    while True:
        ...
        # Counts how many cars have passed the ROI. Not working properly
        if len(boxes) > 0:
            i = int(0)
            for box in boxes:
        ...
        # Sends a message to the other jetson that it's done
        sendMessage(channel, str(frame_count))
