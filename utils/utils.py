...
def clip_coords(boxes, img_shape):
    ...
    # Kent's change
    boxes[:, 0] = np.maximum(0, np.minimum(boxes[:, 0], img_shape[1]))
    boxes[:, 1] = np.maximum(0, np.minimum(boxes[:, 1], img_shape[0]))
    boxes[:, 2] = np.maximum(0, np.minimum(boxes[:, 2], img_shape[1]))
    boxes[:, 3] = np.maximum(0, np.minimum(boxes[:, 3], img_shape[0]))
